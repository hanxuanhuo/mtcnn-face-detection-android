...
//#if _WIN32
//static string modeldir = "model";
//#else
//#if __ANDROID__
static string modeldir = "/mnt/sdcard/mtcnn";
extern ofstream faceROI;
//#endif
//#endif
class Pnet
{
public:
    ...
    float nms_threshold;
    mydataFmt Pthreshold; //define mydataFmt float
    bool firstFlag;
    vector<struct Bbox> boundingBox_;
    vector<orderScore> bboxScore_;   //defined struct 
private:
...
private:
    ...
    void RnetImage2MatrixInit(struct pBox *pbox);  //imageinit???
};
...
public:
    ...
    ~mtcnn();
    int Init(std::string modeldir="/shell/emulated/0/mtcnn");
    //__android_log_print(ANDROID_LOG_INFO,RUN,"/shell/");
