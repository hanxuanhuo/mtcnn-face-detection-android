...
Pnet::Pnet(){
	...
	string filename = modeldir+"/Pnet.txt";
	//__android_log_print(ANDROID_LOG_INFO,RUN,"READ PNET DONE");
...
void Pnet::run(Mat &image, float scale){
    if(firstFlag){
        __android_log_print(ANDROID_LOG_INFO,RUN,"first init");
        image2MatrixInit(image, this->rgb);
        //__android_log_print(ANDROID_LOG_INFO,RUN,"ERROR PLACE 1");
        feature2MatrixInit(this->rgb, this->conv1_matrix, this->conv1_wb);
        //__android_log_print(ANDROID_LOG_INFO,RUN,"ERROR PLACE 2");
        convolutionInit(this->conv1_wb, this->rgb, this->conv1, this->conv1_matrix);
        //__android_log_print(ANDROID_LOG_INFO,RUN,"ERROR PLACE 3");

        maxPoolingInit(this->conv1, this->maxPooling1, 2, 2);
        //__android_log_print(ANDROID_LOG_INFO,RUN,"ERROR PLACE 4");
        feature2MatrixInit(this->maxPooling1, this->maxPooling_matrix, this->conv2_wb);
        //__android_log_print(ANDROID_LOG_INFO,RUN,"ERROR PLACE 5");
        convolutionInit(this->conv2_wb, this->maxPooling1, this->conv2, this->maxPooling_matrix);
        //__android_log_print(ANDROID_LOG_INFO,RUN,"ERROR PLACE 6");
    ...
    //softmax layer
    //__android_log_print(ANDROID_LOG_INFO,RUN,"GENERATEBBOX");
...
void Pnet::generateBbox(const struct pBox *score, const struct pBox *location, mydataFmt scale){
    ...
    //__android_log_print(ANDROID_LOG_INFO,RUN,"score->width: %d",score->width);
    ...
    for(int row=0;row<score->height;row++){
        for(int col=0;col<score->width;col++){
            if(*p>Pthreshold){
                ...
                count++;
                __android_log_print(ANDROID_LOG_INFO,RUN,"bboxScore_ count = %d",count);
            }
            p++;
            plocal++;
        }
...
Rnet::Rnet(){
	...
	string filename = modeldir + "/Rnet.txt";
	//__android_log_print(ANDROID_LOG_INFO,RUN,"READ RNET DONE");
    readData(filename, dataNumber, pointTeam);
...
Onet::Onet(){
	...
	string filename = modeldir + "/Onet.txt";
	//__android_log_print(ANDROID_LOG_INFO,RUN,"READ ONET DONE");
    readData(filename, dataNumber, pointTeam);
...
mtcnn::mtcnn(int imageRow,int imageCol){
    row = imageRow;
    col = imageCol;
    Init();
}
...
int mtcnn::SetMinFaceSize(int size)
{
    minsize = size;
    Init();
    return 0;
...
FaceDetectionResult mtcnn::Detect(const Mat &image, std::vector<FaceInfo> &faces){
    __android_log_print(ANDROID_LOG_INFO,RUN,"START DETECT");
    ...
    //__android_log_print(ANDROID_LOG_INFO,RUN,"JUDGE THE IMAGE SIZE");
    faces.clear();
    struct orderScore order;
    int count = 0;
    for (size_t i = 0; i < scales_.size(); i++) {
        ...
        //__android_log_print(ANDROID_LOG_INFO,RUN,"START RUNNING");
        simpleFace_[i].run(reImage, scales_.at(i));
        //__android_log_print(ANDROID_LOG_INFO,RUN,"RUN DONE");
        ...
        for(vector<struct Bbox>::iterator it=simpleFace_[i].boundingBox_.begin(); it!=simpleFace_[i].boundingBox_.end();it++){
            __android_log_print(ANDROID_LOG_INFO,RUN,"pnet loop");
            if((*it).exist){
            ...
            __android_log_print(ANDROID_LOG_INFO,RUN,"bbox count: %d",count);
        }
        simpleFace_[i].bboxScore_.clear();
        simpleFace_[i].boundingBox_.clear();
    }
    __android_log_print(ANDROID_LOG_INFO,RUN,"FIRST STAGE");
    //the first stage's nms
    if(count<1){
        __android_log_print(ANDROID_LOG_INFO,RUN,"too less bboxes pnet");
        return TOO_LESS_BBOXES_PNET;
    }
    nms(firstBbox_, firstOrderScore_, nms_threshold[0]);
    refineAndSquareBbox(firstBbox_, image.rows, image.cols);
    __android_log_print(ANDROID_LOG_INFO,RUN,"first stage done");
    //second stage
    count = 0;
    ...
    __android_log_print(ANDROID_LOG_INFO,RUN,"SECOND STAGE DONE");
    ...
    __android_log_print(ANDROID_LOG_INFO,RUN,"THIRD STAGE DONE");
	
	vector<Point> tls;
	vector<Point> bts;
	
    if(count<1){
        __android_log_print(ANDROID_LOG_INFO,RUN,"no face exists");
        return TOO_LESS_BBOXES_ONET;
    }
	
    refineAndSquareBbox(thirdBbox_, image.rows, image.cols);
    nms(thirdBbox_, thirdBboxScore_, nms_threshold[2], "Min");

    __android_log_print(ANDROID_LOG_INFO,RUN,"START DRAWING");
    for(vector<struct Bbox>::iterator it=thirdBbox_.begin(); it!=thirdBbox_.end();it++){
        ...
        if((*it).exist){
            ...
            faces.push_back(fi);
			//===========
			Point tl((*it).y1, (*it).x1);
			Point bt((*it).y2, (*it).x2);
			tls.push_back(tl);
			bts.push_back(bt);
	...
	//put the facebbox information into a file
	//ofstream faceInfo(modeldir + "/FaceRect.txt",ios::out|ios::app);

	//added
	for (int num = 0; num < tls.size(); num++) {
	    __android_log_print(ANDROID_LOG_INFO,RUN,"drawing...");
		rectangle(image, tls[num], bts[num], Scalar(0, 255, 0), 2, 8, 0);
		//circle(image, Point(faces[num].bbox.x, faces[num].bbox.y), 3, Scalar(0, 255, 255), -1);
		faceROI << tls[num].x << "," << tls[num].y << ";" << bts[num].x << "," << bts[num].y << "\\";
	}
	__android_log_print(ANDROID_LOG_INFO,RUN,"DRAW DONE");
	count = 0;
    firstBbox_.clear();
	...
	tls.clear();
	bts.clear();
	__android_log_print(ANDROID_LOG_INFO,RUN,"ALL CLEAR");
    return FaceDetectionResult::OK;
}