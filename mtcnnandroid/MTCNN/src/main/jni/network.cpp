...
void addbias(struct pBox *pbox, mydataFmt *pbias){
    if (pbox->pdata == NULL){
        __android_log_print(ANDROID_LOG_INFO,RUN,"Relu feature is Null");
        //cout << "Relu feature is NULL!!" << endl;
        return;
    }
    if (pbias == NULL){
        __android_log_print(ANDROID_LOG_INFO,RUN,"the Relu bias is Null");
...
void image2MatrixInit(Mat &image, struct pBox *pbox){
    if ((image.data == NULL) || (image.type() != CV_8UC3)){
        __android_log_print(ANDROID_LOG_INFO,RUN,"image's type is wrong,Please set CV_8UC3");
    ...
    if(pbox->pdata==NULL)__android_log_print(ANDROID_LOG_INFO,RUN,"the image2MatrixInit failed");
...
void image2Matrix(const Mat &image, const struct pBox *pbox){
    ...
    if (pbox->pdata == NULL){
        __android_log_print(ANDROID_LOG_INFO,RUN,"image2Matrix pdata is Null");
        return;
    }
...
void featurePadInit(const pBox *pbox, pBox *outpBox, const int pad){
    if (pad <= 0){
        __android_log_print(ANDROID_LOG_INFO,RUN,"the data needn't to pad,please check your network");
    ...
    if(outpBox->pdata==NULL)__android_log_print(ANDROID_LOG_INFO,RUN,"the featurePadInit is failed");
...
void feature2MatrixInit(const pBox *pbox, pBox *Matrix, const Weight *weight){
   ...
   //__android_log_print(ANDROID_LOG_INFO,RUN,"FEATURE2MATRIX SET SIZE");
    ...
    //__android_log_print(ANDROID_LOG_INFO,RUN,"SET PROPERITIES");
    if(Matrix->pdata==NULL)__android_log_print(ANDROID_LOG_INFO,RUN,"The feature2MatrixInit failed!");
    memset(Matrix->pdata, 0, Matrix->width*Matrix->height*sizeof(mydataFmt));
    //__android_log_print(ANDROID_LOG_INFO,RUN,"MEMSET");
}
void feature2Matrix(const pBox *pbox, pBox *Matrix, const Weight *weight){
    if (pbox->pdata == NULL){
        __android_log_print(ANDROID_LOG_INFO,RUN,"the feature2Matrix pbox is NULL");
...
void convolutionInit(const Weight *weight, const pBox *pbox, pBox *outpBox, const struct pBox *matrix){
    ...
    if(outpBox->pdata==NULL)__android_log_print(ANDROID_LOG_INFO,RUN,"the convolutionInit id failed");
...
void convolution(const Weight *weight, const pBox *pbox, pBox *outpBox, const struct pBox *matrix){
    if (pbox->pdata == NULL){
        __android_log_print(ANDROID_LOG_INFO, RUN,"the feature is Null");
        //cout << "the feature is NULL!!" << endl;
        return;
    }
    if (weight->pdata == NULL){
        __android_log_print(ANDROID_LOG_INFO,RUN,"the weight is Null");
...
void maxPoolingInit(const pBox *pbox, pBox *Matrix, int kernelSize, int stride){
    ...
    if(Matrix->pdata==NULL)__android_log_print(ANDROID_LOG_INFO,RUN,"the maxPoolingInit is failed");
...
void relu(struct pBox *pbox, mydataFmt *pbias){
    if (pbox->pdata == NULL){
        __android_log_print(ANDROID_LOG_INFO,RUN,"the Relu features is Null");
...
void prelu(struct pBox *pbox, mydataFmt *pbias, mydataFmt *prelu_gmma){
    if (pbox->pdata == NULL){
        __android_log_print(ANDROID_LOG_INFO,RUN,"the Relu feature is Null");
        //cout << "the  Relu feature is NULL!!" << endl;
        return;
    }
    if (pbias == NULL){
...
void fullconnectInit(const Weight *weight, pBox *outpBox){
    ...
    if(outpBox->pdata==NULL)__android_log_print(ANDROID_LOG_INFO,RUN,"the fullconnectInit is failed!!");
    //cout<<"the fullconnectInit is failed!!"<<endl;
...
void fullconnect(const Weight *weight, const pBox *pbox, pBox *outpBox){
    if (pbox->pdata == NULL){
        __android_log_print(ANDROID_LOG_INFO,RUN,"the fc feature is NULL!!");
        //cout << "the fc feature is NULL!!" << endl;
        return;
    }
    if (weight->pdata == NULL){
        __android_log_print(ANDROID_LOG_INFO,RUN,"the fc weight is NULL!!");
...
void readData(string filename, long dataNumber[], mydataFmt *pTeam[]){
    __android_log_print(ANDROID_LOG_INFO,RUN,"read Data from %s",filename.c_str());
    ...
    {  
        ...
        {   
            try{
                ...
                __android_log_print(ANDROID_LOG_INFO,RUN,"EXCEPTION %d",i);
                //cout<<" yichang "<<i<<endl;
                return;
            }
        } 
    ...
    else 
    {
        ...
        __android_log_print(ANDROID_LOG_INFO,RUN,"%s",msg.c_str());
...
long initConvAndFc(struct Weight *weight, int schannel, int lchannel, int kersize, int stride, int pad){
    ...
    if(weight->pbias==NULL)__android_log_print(ANDROID_LOG_INFO,RUN,"memory alloc error");
    ...
    if(weight->pdata==NULL)__android_log_print(ANDROID_LOG_INFO,RUN,"memory alloc error");
    //cout<<"neicun muyou shenqing chengong!!";
...
void initpRelu(struct pRelu *prelu, int width){
    ...
    if(prelu->pdata==NULL)__android_log_print(ANDROID_LOG_INFO,RUN,"prelu apply for memory failed");
    //cout<<"prelu apply for memory failed!!!!";
    memset(prelu->pdata, 0, width*sizeof(mydataFmt));
}
void softmax(const struct pBox *pbox){
    if(pbox->pdata==NULL){
        __android_log_print(ANDROID_LOG_INFO,RUN,"the softmax's pdata is Null,please check!");
...
void nms(vector<struct Bbox> &boundingBox_, vector<struct orderScore> &bboxScore_, const float overlap_threshold, string modelname){
    if(boundingBox_.empty()){
        __android_log_print(ANDROID_LOG_INFO,RUN,"bboxes is empty");
        return;
    }
...
void refineAndSquareBbox(vector<struct Bbox> &vecBbox, const int &height, const int &width){
    if(vecBbox.empty()){
        __android_log_print(ANDROID_LOG_INFO,RUN,"Bbox is empty");
